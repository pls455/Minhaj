...
function installDuplicateControl(root=document){const content=root.getElementById?.('adminContent')||document.getElementById('adminContent');if(!content)return;const observer=new MutationObserver(()=>{const heading=[...content.querySelectorAll('h2')].find(x=>x.textContent?.trim()==='سجل المصادر');if(!heading)return;const head=heading.closest('.section-head');if(!head||head.dataset.duplicateControl==='1')return;head.dataset.duplicateControl='1';const b=document.createElement('button');b.type='button';b.className='button';b.textContent='كشف التكرار';b.addEventListener('click',refreshScan);head.append(b)});observer.observe(content,{subtree:true,childList:true});}

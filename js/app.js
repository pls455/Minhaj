...
async function getBranches(){
  const key="minhaj:branches:public";
  const cached=localStorage.getItem(key);
  try{
    const snap=await getDocs(collection(db,"branches"));
    const data=snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>x.active!==false).sort((a,b)=>(a.order??9999)-(b.order??9999)||String(a.name||"").localeCompare(String(b.name||""),"ar"));
    localStorage.setItem(key,JSON.stringify(data));
    return data;
  }catch(e){ return cached?JSON.parse(cached):[]; }
}

async function renderHomeBranches(){
  const grid=$("#branchesGrid"); if(!grid)return;
  const data=await getBranches();
  const icons=["🔬","📖","⚙️","💻","🎓","📚"];
  grid.innerHTML=data.length?data.map((b,i)=>`<a href="subjects.html?branch=${encodeURIComponent(b.id)}" class="branch"><small>${String(i+1).padStart(2,"0")}</small><div class="branch-icon">${esc(b.icon||icons[i%icons.length])}</div><h3>${esc(b.name)}</h3><p>${esc(b.description||`مواد ومصادر فرع ${b.name}`)}</p></a>`).join(""): `<a href="subjects.html" class="branch"><small>01</small><div class="branch-icon">📚</div><h3>كل الفروع</h3><p>استعرض المواد والمصادر التعليمية</p></a>`;
}
...
async function renderSubjects(){
  ...
  $$("[data-branch]").forEach(b=>{b.classList.add("active"); b.onclick=()=>{branch=""; history.replaceState({},"","subjects.html"); render();};});
...
async function renderFoundations(){
  const grid=$("#foundationsGrid"); if(!grid)return; const branchSel=$("#foundationBranch"), subjectSel=$("#foundationSubject"), levelSel=$("#foundationLevel"), typeSel=$("#foundationType"), search=$("#foundationSearch"); let branch=new URLSearchParams(location.search).get("branch")||""; branchSel.value=""; const lockedBranch=branch;
  ...
  async function reload(){branch=lockedBranch||branchSel.value; await refreshSubjects(); data=await getFoundations({branch,subject:subjectSel.value}); render();}
...
setupNav(); renderHomeBranches(); renderSubjects(); renderResources(); renderFoundations();
